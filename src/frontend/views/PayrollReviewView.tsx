    ...
    <div className="space-y-6">
      ...
      <PayrollMetricSummaryCards
        cards={[
          {
            label: "当前校区筛选课节数",
            value: `${filteredLessons.length} 节`,
            hint: `计薪完成 ${campusHours.toFixed(1)}h · 未完成 ${campusUnfinishedHours.toFixed(1)}h`
          },
          {
            label: "当前校区扣前课时费",
            value: formatPrivateMoney(campusLessonFee, amountsVisible),
            hint: "完成及补课完成 · 义务课时抵扣前"
          },
          {
            label: "义务课时扣费",
            value: `-${formatPrivateMoney(campusDeduction, amountsVisible)}`,
            hint: !obligationDeductionApplies
              ? `抵扣归入 ${campusName(vault, effectiveObligationCampusId)}`
              : currentCampusObligation.mode === "manual"
              ? "手动填写扣除"
              : `应扣 ${currentCampusObligation.requiredHours.toFixed(1)}h · 已扣 ${obligationCourseDeductedHours.toFixed(1)}h · 未抵 ${currentCampusObligation.missingHours.toFixed(1)}h`,
            danger: true
          },
          {
            label: "当前校区扣后课时费",
            value: formatPrivateMoney(campusNet, amountsVisible),
            hint: campusFilter === "all" ? "全部校区筛选 · 义务课时抵扣后" : `${campusName(vault, campusFilter)} · 义务课时抵扣后`
          },
          {
            label: "各校区扣前课时费总计",
            value: formatPrivateMoney(lessonFeeTotal, amountsVisible),
            hint: "本月各校区完成课时费汇总"
          },
          {
            label: "本月预估收入",
            value: formatPrivateMoney(estimatedIncome, amountsVisible),
            hint: "含待上课课节预估"
          }
        ]}
      />
