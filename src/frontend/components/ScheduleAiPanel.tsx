            ...
            <div className="space-y-4">
              <div className="rounded-[14px] border border-[#dbe4ef] bg-white p-4">
                ...
                <div className="grid grid-cols-1 gap-2 text-sm font-semibold text-[#25324a]">
                  {[
                    "查询本周、本月、指定日期或时间段的课节数量、日期分布和待处理课时",
                    "按现有课节金额汇总已知课时费；缺少金额或规则不完整时会明确提示无法计算",
                    "新增学生，或修改学生姓名、年级、校区、学校、备注",
                    "新增课程档案，并设置科目、班型、校区、关联学生和课时费来源",
                    "新增自定义班型，并带入班课或非班课备用计费模板",
                    "修改课程班型、校区、关联学生、课时费规则，或迁移课程/课节到新课程",
                    "按指定日期、星期或日期范围批量新增排课，并做冲突校验",
                    "同步某一天或某段日期的排课到目标日期，可选择是否覆盖已有课节",
                    "删除、取消或暂停明确匹配到的课程/课节；范围不明确时会要求补充信息",
                    "所有写入类建议都必须先预览并人工确认，AI 不会直接改数据"
                  ].map((text) => (
              ...
              <div className="rounded-[14px] border border-[#fed7aa] bg-[#fff7ed] p-4">
                ...
                <div className="text-sm font-semibold leading-6 text-[#9a3412]">
                  今日提醒、补课安排、单节课程内容/作业记录、冲突裁决和工资最终核对继续由系统代码与人工确认处理；涉及写入的数据必须先生成建议并由人工确认。
                </div>
              </div>
