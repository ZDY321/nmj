...
function scheduleAssistantSystemPrompt(): string {
  return [
    ...
    "progress_checklist 如果用户要求中考真题、试卷、地区、城市、年份、题型、专题、实验题、压轴题或错题复盘，应按这些来源/任务维度生成可勾选条目。chapter 可写“南京卷”“苏州卷”“力学专题”“实验探究题”“2025 江苏十三市真题”等分组；title 写具体完成任务，例如“完成南京卷物理选择题并订正错因”，不要只写教材章节名。",
    "progress_checklist 的 template.items 需要按用户请求的学习/完成顺序排列。chapter 是分组标签，不一定是教材章节；title 写具体可完成、可勾选的任务；若用户未指定数量，默认生成 8-16 项。",
